...
void core1_task() {
    while (1) {
        /*
        uint16_t btndata = current_con.Button;
        for (uint8_t i=0; i<8; i++) {
            feedback_pixel(urgb_u32(0, 0, btndata&1));
            btndata >>= 1;
        }
        */
    ...
    while (uart_is_readable(UART_ID)) {
        ...
        // parse the full command on newline
            ...
            if (strncmp(cmd_str, "SLAG ", 5) == 0) {
                ...
                if (lag_amount > 120) lag_amount = 120;
            ...
            if (cmd_str_ind < (sizeof(cmd_str)-1)) { // -1 to maintain null termination
...
int add_to_queue(const char* cstr){
    ...
    for(uint8_t i=7; i<14; i++) {
        // error on any non-hex characters
        if ( !( (cstr[i]>='0' && cstr[i]<='9') || (cstr[i]>='A' && cstr[i]<='F') ) ) hasStick = false;
    }
...
static void alarm_irq(void) {
    ...
    if (action_mode == A_REC) {
        ...
        // increment index
        if (rec_head < sizeof(rec_data_buff)) rec_head++;
    }
    ...
    // If playing in lag mode, move the queue pointers and send the next entry.
        ...
        queue_head = (queue_head+1) % CON_BUFF_LEN;
        if (queue_tail < queue_head) { // Head is not wrapped
            if ((queue_head-queue_tail) > lag_amount) { // lag at limit; tail needs to keep up
                queue_tail = (queue_tail+1) % CON_BUFF_LEN;
            }
        } else { // Head is wrapped; need to account for it.
